...
import (
	...
	sqrl "github.com/Khulnasoft-lab/goctlsql"
...
const (
	testSchema = `
		CREATE TABLE goctlsql_integration ( k INT, v TEXT )`
	testData = `
		INSERT INTO goctlsql_integration VALUES
			(1, 'foo'),
			(3, 'bar'),
			(2, 'foo'),
			(4, 'baz')
...
func TestMain(m *testing.M) {
	...
	defer func() {
		_, err = db.Exec("DROP TABLE goctlsql_integration")
...
func TestSimpleSelect(t *testing.T) {
	assertVals(
		t,
		sb.Select("v").From("goctlsql_integration"),
		"foo", "bar", "foo", "baz")
}

func TestEq(t *testing.T) {
	s := sb.Select("v").From("goctlsql_integration")
